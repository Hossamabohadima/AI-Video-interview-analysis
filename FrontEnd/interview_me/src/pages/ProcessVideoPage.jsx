...
function AnalyzeIcon() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="-3 0 30 24" fill="none"
      stroke="#0FA99D" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5 shrink-0">
...
function HistoryIcon() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none"
      stroke="#0FA99D" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" className="h-4 w-4 shrink-0">
...
function UploadCloudIcon() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="-3 0 30 24" fill="none"
      stroke="#9CA3AF" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" className="h-8 w-8 shrink-0">
      <path d="M20 16.5A4.5 4.5 0 0 0 17 8h-1.26A8 8 0 1 0 4 15.25" />
      <path d="M12 20V11" />
      <path d="m8.8 14.2 3.2-3.2 3.2 3.2" />
    </svg>
...
function FileVideoIcon() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none"
      stroke="#9CA3AF" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-4 w-4">
  ...
  return (
    <div className="rounded-[18px] bg-white px-6 py-5 shadow-sm">
      ...
      <input
        ...
        className={`h-[42px] w-full rounded-full border px-4 text-[12px] outline-none placeholder:text-[#B0B0B0] bg-[#F5F5F5] ${
          !roleName.trim() ? "border-red-300" : "border-[#D1D5DB]"
        }`}
      />
    </div>
  );
...
function AnalysisWeightsCard({ weights, setWeights, threshold, setThreshold, isRecruiter, weightError }) {
  const total = Object.values(weights).reduce((sum, val) => sum + val, 0);
  const updateWeight = (key, nextValue) => setWeights((prev) => ({ ...prev, [key]: nextValue }));
  ...
  return (
    <div className="rounded-[18px] bg-white px-6 py-5 shadow-sm">
      ...
      <div className="space-y-5">
        <SliderRow label="Fillers Usage"      value={weights.fillers}    onChange={(v) => updateWeight("fillers", v)} />
        <SliderRow label="Pause Duration"     value={weights.pause}      onChange={(v) => updateWeight("pause", v)} />
        <SliderRow label="Emotion Intensity"  value={weights.emotion}    onChange={(v) => updateWeight("emotion", v)} />
        <SliderRow label="Energy Level"       value={weights.energy}     onChange={(v) => updateWeight("energy", v)} />
        <SliderRow label="Eye Contact"        value={weights.eyeContact} onChange={(v) => updateWeight("eyeContact", v)} />
        <SliderRow label="Grammar Accuracy"   value={weights.grammar}    onChange={(v) => updateWeight("grammar", v)} />
      </div>
      <div className="mt-6 rounded-xl bg-[#F8FAFA] px-4 py-3 text-center">
        <div className={`text-[14px] font-semibold ${total === 100 ? "text-[#0FA99D]" : "text-red-500"}`}>
          Total: {total}%
        </div>
        {weightError && <p className="mt-2 text-[13px] text-red-500">{weightError}</p>}
      </div>
      {isRecruiter && (
...
function UploadDropzone({ onFilesAdded, isDragging, setIsDragging, inputRef, isRecruiter }) {
  const handleFiles = (fileList) => {
    const allFiles   = Array.from(fileList || []);
    const videoFiles = allFiles.filter((file) => file.type.startsWith("video/"));
    if (!videoFiles.length) return;
    if (isRecruiter) { onFilesAdded(videoFiles); } else { onFilesAdded([videoFiles[0]]); }
  };
  ...
  return (
    <div
      ...
      className={`flex min-h-[150px] cursor-pointer flex-col items-center justify-center rounded-[18px] border bg-white p-5 text-center shadow-sm transition ${
        isDragging ? "border-[#0FA99D] bg-[#F0FDFA]" : "border-dashed border-[#AFAFAF]"
      }`}
    >
      <input ref={inputRef} type="file" accept="video/*" multiple={isRecruiter} className="hidden"
        onChange={(e) => handleFiles(e.target.files)} />
      <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-full bg-[#F3F4F6]">
        <UploadCloudIcon />
      </div>
      <h3 className="text-[18px] font-bold text-[#7C7C7C]">Drag & drop candidates videos</h3>
      <p className="mt-3 text-[13px] text-[#A1A1AA]">Upload MP4, MOV, or WEBM files.</p>
      {isRecruiter
        ? <p className="text-[13px] text-[#A1A1AA]">You can upload up to 300 videos at once for bulk processing.</p>
        : <p className="text-[13px] text-[#A1A1AA]">You can upload only one video.</p>
      }
      <p className="mt-3 text-[12px] font-medium text-[#0FA99D]">Click here to browse files</p>
    </div>
  );
}
  ...
  return (
    <div className="flex items-center justify-between rounded-[14px] border border-[#E5E7EB] bg-white px-4 py-3">
      <div className="flex min-w-0 items-center gap-3">
        ...
        <span className="truncate text-[14px] font-medium text-[#6B7280]">{file.name}</span>
      </div>
      <div className="ml-4 flex items-center gap-4">
        <SmallUploadStatus />
        <button type="button" className="text-[#D1D5DB]" onClick={() => onRemove(file.id)}>✕</button>
      </div>
    </div>
  );
}
    ...
    <div className="rounded-[18px] bg-white px-5 py-5 shadow-sm">
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <h3 className="text-[18px] font-bold text-[#7C7C7C]">Ready to process</h3>
          <span className="flex h-5 w-5 items-center justify-center rounded-full bg-[#E5E7EB] text-[11px] text-[#7C7C7C]">
            {files.length}
          </span>
        </div>
        <button type="button" className="text-[12px] text-[#7C7C7C]" onClick={clearAllFiles} disabled={!files.length}>
          Clear all
        </button>
      </div>
      <div className="space-y-3">
        {files.length ? (
          files.slice(0, 4).map((file) => (
            <UploadedFileRow key={file.id} file={file} onRemove={removeFile} />
          ))
        ) : (
      ...
      {files.length > 4 && (
        <div className="mt-4 text-center text-[13px] text-[#7C7C7C]">View more</div>
      )}
    </div>
  );
}
  ...
  return (
    <button type="button" disabled={disabled || loading} onClick={onClick}
      className={`flex h-[58px] w-full items-center justify-center gap-3 rounded-[14px] text-[18px] font-bold text-white shadow-sm ${
        disabled || loading ? "cursor-not-allowed bg-[#9FDCD5]" : "bg-[#0FA99D] hover:bg-[#0c8f85]"
      }`}
    >
...
function ProcessVideoPage() {
  ...
  const profileChar  =
    trimmedName.split(" ").length >= 2
      ? trimmedName.split(" ").slice(0, 2).map((n) => n[0]).join("").toUpperCase()
      : trimmedName.slice(0, 2).toUpperCase();

  const isRecruiter = role === "RECRUITER";

  const [weights, setWeights] = useState({
    fillers: 30, pause: 20, emotion: 20, energy: 10, eyeContact: 10, grammar: 10,
  });
  ...
  const onFilesAdded = (newFiles) => {
    ...
    if (isRecruiter) { setFiles((prev) => [...prev, ...prepared]); }
    else             { setFiles(prepared.slice(0, 1)); }
  };
  ...
  useEffect(() => {
    if (totalWeight === 100) { setWeightError(""); }
    else                     { setWeightError("Weights must sum up to 100%"); }
  }, [totalWeight]);
  ...
  const handleStartProcessing = async () => {
    ...
    if (!roleName.trim()) {
      alert("Please enter a Target Role Name before processing.");
      return;
    }
    ...
    try {
      ...
      formData.append("eye_contact_weight", (weights.eyeContact / 100).toFixed(2));
      formData.append("grammar_weight",     (weights.grammar    / 100).toFixed(2));

      const response = await fetch("http://127.0.0.1:8000/users/videos/upload", {
        method:  "POST",
      ...
      // ── Store batch role map (videoId → roleName) for history page ──
      const batchIds     = data.uploaded_videos ? data.uploaded_videos.map((v) => v.video_id) : [];
      const batchRoleMap = JSON.parse(localStorage.getItem("batchRoleMap") || "{}");
      batchIds.forEach((id) => { batchRoleMap[String(id)] = roleName; });
      localStorage.setItem("batchRoleMap",        JSON.stringify(batchRoleMap));
    ...
    <div className="min-h-screen w-full bg-[#E8E6E2]">
      <style>{`
        .slider-teal::-webkit-slider-thumb {
          ...
          border-radius: 9999px; background: #0FA99D;
          cursor: pointer; border: none; margin-top: -5px;
        }
        .slider-teal::-moz-range-thumb {
          width: 16px; height: 16px; border-radius: 9999px;
          background: #0FA99D; cursor: pointer; border: none;
        }
      `}</style>
      ...
      <header className="flex h-[72px] items-center justify-between border-b border-[#D8D8D8] bg-white px-6">
        ...
        <div className="flex items-center gap-4">
          <div className="border-r border-[#E5E7EB] pr-3 text-right">
            <div className="text-[12px] font-semibold text-[#374151]">{name}</div>
            <div className="text-[10px] text-[#9CA3AF]">{isRecruiter ? "Recruiter Admin" : "User"}</div>
          </div>
          <div ref={profileMenuRef} className="relative">
            <button type="button" onClick={() => setProfileMenuOpen((prev) => !prev)}
      ...
      <div className="flex">
        <aside className="min-h-[calc(100vh-72px)] w-[240px] bg-white">
          <nav className="mt-10 flex flex-col gap-2 px-3">
            <SidebarItem active icon={<AnalyzeIcon />} label="Analyze Interview" to="/process-video" />
            <SidebarItem icon={<HistoryIcon />} label="History"
              to={isRecruiter ? "/recruiter-history" : "/candidate-history"} />
          </nav>
        </aside>

        <main className="flex-1 bg-[#E8E6E2] px-6 py-6">
          <div className="mb-6">
            <h1 className="text-[24px] font-bold text-[#0FA99D]">New Processing</h1>
            {isRecruiter && (
          ...
          <div className="grid grid-cols-[1fr_1.2fr] gap-6">
            <div className="space-y-5">
              ...
              <AnalysisWeightsCard
                ...
                isRecruiter={isRecruiter} weightError={weightError}
              />
            </div>
            <div className="space-y-5">
              <UploadDropzone
                onFilesAdded={onFilesAdded} isDragging={isDragging}
                setIsDragging={setIsDragging} inputRef={inputRef} isRecruiter={isRecruiter}
              />
              <ReadyToProcessCard files={files} clearAllFiles={clearAllFiles} removeFile={removeFile} />
              <StartProcessingButton disabled={!files.length} onClick={handleStartProcessing} loading={loading} />
            </div>
          </div>
